...
void CClothSimulationTask::ComputeGPU(cl_context, cl_command_queue CommandQueue, size_t LocalWorkSize[3])
{
	...
	V_RETURN_CL(clErr, "Error executing integrate collisions kernel");
	...
	V_RETURN_CL(clErr, "Failed to set m_CollisionsKernel kernel params");
	clErr = clEnqueueNDRangeKernel(CommandQueue, m_CollisionsKernel, 2, NULL, globalWorkSize, LocalWorkSize, 0, NULL, NULL);
	V_RETURN_CL(clErr, "Error executing m_CollisionsKernel kernel");
	...
	for (unsigned int i = 0; i < 2.0 * m_ClothResX; i++) {
		...
		V_RETURN_CL(clErr, "Error executing m_ConstraintKernel kernel");

		// Swap the ping pong buffers
		std::swap(m_clPosArray, m_clPosArrayAux);

		// Occasionally check for collisions and on last run
		if (i % 3 == 0 || i + 1 == 2 * m_ClothResX) {
			clErr = clSetKernelArg(m_CollisionsKernel, 2, sizeof(cl_mem), (void*)&m_clPosArray);
			V_RETURN_CL(clErr, "Failed to set m_CollisionsKernel kernel params");
			clErr = clEnqueueNDRangeKernel(CommandQueue, m_CollisionsKernel, 2, NULL, globalWorkSize, LocalWorkSize, 0, NULL, NULL);
			V_RETURN_CL(clErr, "Error executing m_CollisionsKernel kernel");
