...
bool CParticleSystemTask::InitResources(cl_device_id Device, cl_context Context)
{
	...
	for (unsigned int i = 0; i < m_nLevels; i++) {
		...
		//clError |= clError2;
		N = (N / (2 * m_LocalWorkSize[0]))>=( m_LocalWorkSize[0])? (N / (2 * m_LocalWorkSize[0])): m_LocalWorkSize[0];
	}
	V_RETURN_FALSE_CL(clError2, "Error allocating device arrays");
	//Load mesh
	...
	for(unsigned int i = 0; i < m_nParticles; i++) {
		...
		pPosLife[i].s[3] = 10.f + 5.f * (float(rand()) / float(RAND_MAX));
	...
	// Scan arrays
	unsigned int NN = m_nParticles * 2;
	for (unsigned int i = 0; i < m_nLevels; i++) {
		m_clLevelArrays[i] = clCreateBuffer(Context, CL_MEM_READ_WRITE, sizeof(cl_uint) * NN, NULL, &clError2);
		clError |= clError2;
		NN = std::max(NN / (2 * m_LocalWorkSize[0]), m_LocalWorkSize[0]);
	}
	V_RETURN_FALSE_CL(clError, "Error allocating device arrays");
...
void printBuf(cl_command_queue CommandQueue, cl_mem clBuf, size_t offset = 0, size_t count = 10) {
	cl_uint* buf = new cl_uint[count];
	V_RETURN_CL(clEnqueueReadBuffer(CommandQueue, clBuf, CL_TRUE, offset * sizeof(cl_uint), count * sizeof(cl_uint), buf, 0, NULL, NULL), "Error reading data from device!");
...
void CParticleSystemTask::Scan(cl_context , cl_command_queue CommandQueue, size_t LocalWorkSize[3])
{
	...
	uint m_N = m_nParticles * 2;
	size_t globalWorkSize[1];
	globalWorkSize[0] = CLUtil::GetGlobalWorkSize(m_N, LocalWorkSize[0]);

	// copy alive data over
	clErr = clEnqueueCopyBuffer(CommandQueue, m_clAlive, m_clPingArray, 0, 0, m_nParticles * sizeof(cl_uint) * 2, 0, NULL, NULL);
	V_RETURN_CL(clErr, "Copy buffer failed");

	for(uint i = 0; (1 << i) < m_N; i++) {
		size_t offset = (1 << i);

		//cout << "Enqueue " << globalWorkSize[0] << " offset " << offset << endl;
		...
		clErr |= clEnqueueNDRangeKernel(CommandQueue, m_ScanNaiveKernel,
											1, NULL, globalWorkSize, LocalWorkSize,
											0, NULL, NULL);
		V_RETURN_CL(clErr, "Failed to enqueue kernel: Scan_Naive.");

		std::swap(m_clPingArray, m_clPongArray);
	}

	//printBuf(CommandQueue, m_clPingArray, 500, 20);
}

void CParticleSystemTask::Scan_efficient(cl_context, cl_command_queue CommandQueue, size_t LocalWorkSize[3]) {
	cl_int clErr;
	uint m_N = m_nParticles * 2;
	size_t globalWorkSize[1];
	unsigned int N = m_N / 2;
	clErr = clEnqueueCopyBuffer(CommandQueue, m_clAlive, m_clLevelArrays[0], 0, 0, m_nParticles * sizeof(cl_uint) * 2, 0, NULL, NULL);
	V_RETURN_CL(clErr, "Copy buffer failed");
	
	for (unsigned int i = 0; i < m_nLevels - 1; i++) {
		globalWorkSize[0] = N;
		//cout << "Enqueue Reduce " << i << " with " << N << "(groups: " << (N/LocalWorkSize[0]) << ")" << endl;
		clErr = clSetKernelArg(m_ScanKernel, 0, sizeof(cl_mem), (void*)&m_clLevelArrays[i]);
		clErr |= clSetKernelArg(m_ScanKernel, 1, sizeof(cl_mem), (void*)&m_clLevelArrays[i + 1]);
		//clErr |= clSetKernelArg(m_ScanWorkEfficientKernel, 2, (2 * LocalWorkSize[0] + LocalWorkSize[0]/15) * sizeof(cl_uint), NULL);
		clErr |= clSetKernelArg(m_ScanKernel, 2, (4 * LocalWorkSize[0]) * sizeof(cl_uint), NULL);
		clErr |= clEnqueueNDRangeKernel(CommandQueue, m_ScanKernel,
			1, NULL, globalWorkSize, LocalWorkSize,
			0, NULL, NULL);
		V_RETURN_CL(clErr, "Failed to enqueue kernel: Scan_WorkEfficient.");
		N = (N / (2 * LocalWorkSize[0])) >= (LocalWorkSize[0]) ? (N / (2 * LocalWorkSize[0])) : LocalWorkSize[0];
		//N = max(N / (2 * LocalWorkSize[0]), LocalWorkSize[0]);
	}

	for (unsigned int i = m_nLevels - 1; i > 1; i--) {
		unsigned int currentNumber;
		if (i > 2) {
			currentNumber = m_N / ((i - 2) * 2 * LocalWorkSize[0]);
		}
		else {
			currentNumber = m_N;
		}
		//cout << i - 1 << "     " << currentNumber<< endl;
		globalWorkSize[0] = currentNumber;
		//cout << "Enqueue Add " << (i-1) << " with " << currentNumber << "(groups: " << (currentNumber/LocalWorkSize[0]) << ")"  << endl;

		clErr = clSetKernelArg(m_ScanAddKernel, 0, sizeof(cl_mem), (void*)&m_clLevelArrays[i - 1]);
		clErr |= clSetKernelArg(m_ScanAddKernel, 1, sizeof(cl_mem), (void*)&m_clLevelArrays[i - 2]);
		//clErr |= clSetKernelArg(m_ScanWorkEfficientAddKernel, 2, (2 * LocalWorkSize[0] + LocalWorkSize[0]/15) * sizeof(cl_uint), NULL);
		clErr |= clSetKernelArg(m_ScanAddKernel, 2, (3 * LocalWorkSize[0]) * sizeof(cl_uint), NULL);
		clErr |= clEnqueueNDRangeKernel(CommandQueue, m_ScanAddKernel,
			1, NULL, globalWorkSize, LocalWorkSize,
			0, NULL, NULL);
		V_RETURN_CL(clErr, "Failed to enqueue kernel: Scan_WorkEfficientAdd.");
...
void CParticleSystemTask::Integrate(cl_context , cl_command_queue CommandQueue, size_t LocalWorkSize[3], float dT)
{
	...
	cl_ulong randSeed = rand();
	clErr |= clSetKernelArg(m_IntegrateKernel, 10, sizeof(cl_ulong), (void*)&randSeed);
...
void CParticleSystemTask::Reorganize(cl_context , cl_command_queue CommandQueue, size_t LocalWorkSize[3])
{
	...
	cl_uint zero = 0;
	clErr = clEnqueueFillBuffer(CommandQueue, m_clAlive, &zero, sizeof(cl_uint), 0, m_nParticles * 2 * sizeof(cl_uint), 0, NULL, NULL);
	V_RETURN_CL(clErr, "Error clear alive");
